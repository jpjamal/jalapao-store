...
`{expires_at, days_left, alert}`, ou `None` quando não há o que ler. O caminho vem de
`TLS_CERT_PATH` e o limiar de `TLS_ALERT_DAYS`. O volume `certificates` é montado no backend
como `/certs:ro`. O dashboard passa a devolver o campo `certificate`, e o painel mostra uma
