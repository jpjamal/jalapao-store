...
| `integrations/meli/publicacao.py` | `publicar`: trava, confere, valida, sobe fotos, cria, descreve, grava o `Listing` |
| `integrations/models.py` | `Listing.draft` (um para um com o rascunho, `SET_NULL`) |
| `catalog/models.py` | permissão `publish_listingdraft` |
| `catalog/drafts_api.py` | `POST {id}/publish`; `published_item_id` no rascunho |
